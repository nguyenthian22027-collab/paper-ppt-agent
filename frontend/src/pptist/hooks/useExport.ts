...
export default () => {
  ...
  // 导出PPTX文件
  const exportPPTX = (_slides: Slide[], masterOverwrite: boolean, ignoreMedia: boolean, writeBlob?: PptxBlobWriter): Promise<void> => {
    exporting.value = true
    const pptx = new pptxgen()
    ...
    for (const slide of _slides) {
      ...
      if (slide.remark) {
        ...
        for (const p of pList) {
          ...
          if (textContent?.trim()) text.push(textContent)
        }
        if (text.length) pptxSlide.addNotes(text.join('\n'))
      }
